...
Vue.mixin({
  methods: {
    ...
    post (route, data, competition) {
      ...
      document.cookie = '_csrf=' + _csrf + '; Domain=' + window.location.hostname
    ...
    get (route, competition) {
      return axios.get(config.API_URL + (competition === false ? '' : ('/competitions/' + this.$store.competition.id)) + route, { withCredentials: true })
    },
    ...
    logout () {
      ...
      document.cookie = 'token =; Max-Age=0; Domain=' + window.location.hostname
      this.reload()
    ...
    updateCompetition (competition) {
      ...
      if (!this.$store.user.admin && this.$store.competition) document.cookie = 'token=' + localStorage['competition_' + this.$store.competition.id] + '; Domain=' + window.location.hostname
      this.reload()
    },
